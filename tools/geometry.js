...
function inspect(model, label) {
  ...
  // Опора с клипсой (кухонная, передний ряд при цоколе) по конструкции
  // должна КАСАТЬСЯ цоколя — именно так клипса его и держит (см. CLIP_REACH
  // в engine.js). Бокс детали 'leg' — грубый обобщённый цилиндр-в-кубе
  // шириной LEG_D (50 мм и для металлической, и для кухонной опоры), а не
  // реальный тонкий ствол Ø29 мм — поэтому у бокса запас на касание меньше,
  // чем у настоящей круглой ножки, и обобщённый бокс слегка «заходит» на
  // бокс цоколя там, где в реальности его достаёт только клипса. Это тот же
  // случай, что и с rodFlange выше — не пересечение деталей, а способ
  // крепления.
  // Пробовали сузить исключение до r.hasClip (не любая leg/plinth пара) —
  // всплыло 10 ложных срабатываний на составных сценариях (ряды, угловые
  // стыки, примыкание к фальш-планке), где клипсу с конкретной опоры уже
  // сняли постобработкой (engine.js, ~строка 3012-3032: «висящие» опоры без
  // планки под ними), а геометрическое положение опоры (zFront) при этом не
  // отъезжает обратно — то есть касание с цоколем соседнего модуля остаётся
  // физически тем же самым «клипса впритык», просто под другим флагом.
  // Разбираться в этом — отдельная задача, не блокирующая текущую правку;
  // пока держим широкое исключение (по kind, без учёта hasClip), оно не
  // маскирует ни одного реального бага на сегодняшней кодовой базе (металл
  // + цоколь сейчас структурно невозможны — engine.js принудительно делает
...
// ложного предупреждения), а не саму нехватку опоры.
{
  ...
  const model = buildModel(Object.assign({}, base, { modules: mods }));
  inspect(model, 'столешница: компакт, стык на длине — 5-я тумба topType:panel (крышка вместо планок)');

  const tops = model.parts.filter((p) => p.kind === 'countertop');
  if (tops.length !== 2) problems.push(`столешница: компакт длинный ряд — деталей countertop ${tops.length} вместо 2`);

  if (model.warnings.some((w) => /под краем нет опоры/.test(w))) {
    problems.push('столешница: ложное предупреждение про компакт-плиту без опоры под стыком — у 5-й тумбы (topType:panel) engine.js строит крышку, опора есть');
  }
  const joints = model.hardwareContext.countertopJoints || [];
