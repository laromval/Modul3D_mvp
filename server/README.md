...
- Добавление материалов/фурнитуры по ссылке на товар (`GET
  /catalog-link-sources`, `POST /catalog-link-parse`, `POST
  /catalog-link-refresh`, см. `ТЗ-ПАРСЕР-МАТЕРИАЛОВ.md`) — сервер парсит
  страницу товара стороннего сайта (сейчас — только `mobilier.md`,
  `src/services/catalogLinkParsers/`) и отдаёт черновик клиенту для
  подтверждения; сохранение по-прежнему идёт через уже существующий `PUT
  /catalog-overrides`.
...
| `CATALOG_LINK_MAX_REFRESH_ITEMS` | Максимум позиций за один вызов `POST /catalog-link-refresh` (по умолчанию 60) — защита от использования эндпоинта как инструмента нагрузки чужого сайта |
...
### `GET /catalog-link-sources`, `POST /catalog-link-parse`, `POST /catalog-link-refresh`

Часть фичи «Добавить материал/фурнитуру по ссылке» (см.
`ТЗ-ПАРСЕР-МАТЕРИАЛОВ.md`). Заголовок `Authorization: Bearer <jwt>`
обязателен на всех трёх — гостям недоступно, как и личные
`catalog_overrides`. Ничего не сохраняют — только парсят страницу
стороннего сайта и отдают черновик; сохранение — отдельным вызовом
`PUT /catalog-overrides` (клиент сам добавляет `sourceUrl`/`sourceSiteId`/
`verifiedAt` в объект позиции).

- `GET /catalog-link-sources` -> `{ sites: [{ id, name, domain }, ...] }` —
  список сайтов, для которых есть парсер (сейчас только `mobilier.md`,
  реестр — `src/services/catalogLinkParsers/registry.js`, единственное
  место, где сайты перечислены — из него же строится и SSRF-проверка
  домена ниже).
- `POST /catalog-link-parse { siteId, url }` -> `{ draft: {...} }`.
  Проверяет, что `url` принадлежит домену `siteId` (сравнение hostname, не
  `includes()` — защита от SSRF), скачивает страницу (таймаут ~9с, до 3
  редиректов, каждый обязан остаться на том же домене, тело ответа
  ограничено 5МБ), парсит через модуль сайта из реестра. Поля черновика
  (только те, что реально нашлись на странице — не выдумываются): `name`,
  `price`, `currency`, `unit`, `sourceUrl`, опционально `sheetW`/`sheetH`/
  `thickness` (из названия товара — единственное место с размерами на
  странице; смысл этих двух чисел зависит от категории, черновик не
  утверждает, что это обязательно ширина/высота листа — см. комментарий в
  `catalogLinkParsers/mobilierMd.js`), `imageUrl`, `article`, `brand`
  (из JSON-LD, надёжнее, чем последний сегмент хлебных крошек — не
  совпадает с ним для фурнитуры), `categoryPath` (из хлебных крошек),
  `inStock`.
  Ошибки: `400` — неизвестный `siteId`, некорректный `url`, `url` с другим
  доменом или редирект на другой домен; `404` — сайт вернул 404 по этой
  ссылке; `502` — сайт недоступен/вернул ошибку/не удалось распознать
  вёрстку; `504` — таймаут. Никогда не `500` с сырым HTML.
- `POST /catalog-link-refresh { items: [{ url, siteId }, ...] }` -> `{
  results: [{ url, siteId, ok: true, draft } | { url, siteId, ok: false,
  error }, ...] }` — батч-версия для кнопки «Обновить цены с сайта», не
  более `CATALOG_LINK_MAX_REFRESH_ITEMS` позиций за раз, обрабатываются
  партиями по 4 параллельно (не заваливаем сторонний сайт разом). Ошибка
  по одной позиции (сняли с продажи, таймаут и т.п.) не прерывает
  обработку остальных — `ok: false` только для этой позиции.
