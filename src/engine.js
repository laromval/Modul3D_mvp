...
function buildModuleParts(p) {
  ...
  const countertopMat = (ct) => {
    ...
    if (ct.material === 'custom') {
      ...
      // значение (HTML min="1" в поле — не защита от ручного/вставленного
      // отрицательного числа) — без этой проверки ниже строилась бы деталь
      // с отрицательной толщиной (ломает геометрию 3D/чертежей).
      if (!(th > 0)) return { found: false };
