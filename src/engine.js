...
function buildModuleParts(p) {
  ...
  if (onLegs && baseH > 0) {
    ...
    // 3D-модели (опора с клипсой.obj) пластина клипсы — на 33,5 мм от оси
    // при видимом диаметре опоры в той же модели 54 мм. Если позиционировать
    // клипсу так, будто выступает только тело опоры (LEG_D/2), пластина
    // «утапливается» в цоколь — регрессия, пойманная по 3D-модели.
    const CLIP_NATIVE_REACH = 33.5;    // мм, вылет пластины клипсы от оси (в исходнике)
    const CLIP_NATIVE_D = 54;          // мм, видимый диаметр опоры в том же исходнике
    const CLIP_REACH = CLIP_NATIVE_REACH * LEG_D / CLIP_NATIVE_D; // мм, вылет при текущем LEG_D
    ...
    // У кухонной опоры с цоколем в упор в планку встаёт не тело опоры,
    // а пластина клипсы (CLIP_REACH) — именно она крепится к цоколю.
    const zFront = (kitchen && hasPlinth)
      ? (D / 2 - PLINTH_SETBACK - t) - CLIP_REACH - 0.5
      : D / 2 - LEG_INSET;
